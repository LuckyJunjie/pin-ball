...
- **TR-1.2.3**: Input system must support the following actions:
  ...
  - `launch_ball`: Space (32) or Down Arrow (4194320)
  - `ui_cancel`: Esc key (for pause)

### 1.3 Input Behavior

- **TR-1.3.1**: Flipper inputs must be "pressed" actions (continuous while held)
- **TR-1.3.2**: Launch input must be "pressed" action (for charging)
- **TR-1.3.3**: Pause input must be "just_pressed" action (single activation)
...
- **TR-3.3.1**: Ball must have collision_mask = 14 (2 + 4 + 8) - collides with Flippers, Walls, Obstacles
