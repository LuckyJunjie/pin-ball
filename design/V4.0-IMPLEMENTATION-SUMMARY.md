...
## Implemented (GDD-complete extension)

- **Zones**: Android Acres, Dino Desert, Google Gallery, Flutter Forest, Sparky Scorch (bumpers, ramp, bonuses) with zone scripts; GameManagerV4.add_score(), add_bonus(), increase_multiplier() (ramp every 5 hits).
- **Bonus ball**: GameManagerV4 5s timer and _spawn_bonus_ball(); multiball indicators (4) animate on Google Word / Dash Nest bonus.
- **Backbox**: Leaderboard via LeaderboardV4, initials input, share via SocialSharingV4; loading/failure states; BackboxManagerV4 state machine.
- **Camera**: Per status (waiting, playing, gameOver) zoom/position in MainV4.
- **Character theme**: Ball texture and leaderboard icon from CharacterThemeManagerV4/AssetLoaderV4; applied at spawn and in initials panel.
- **Mobile controls**: MobileControlsV4 overlay when touchscreen available; left/right flipper zones, tap to launch.
- **Achievements & statistics**: AchievementSystemV4 and StatisticsTrackerV4 wired to game start/end, multiplier, bonus, combo.
- **Tutorial**: TutorialSystemV4.start_tutorial() for first-time players when entering MainV4.
- **Combo bonus**: Combo bonus from ComboSystem applied to round score on round lost.
