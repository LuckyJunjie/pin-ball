# Pinball Game v4.0 – Component Specifications

## Overview

This document specifies each playfield zone and component for v4.0 (I/O Pinball clone). Behaviors (scoring, bonus, multiplier, ball spawn) are mapped to Godot nodes and scripts. Flutter references: [FLUTTER-PINBALL-PARSING.md](FLUTTER-PINBALL-PARSING.md).
...
- **Script:** LauncherV4.gd – exposes spawn position; handles launch input and notifies GameManagerV4 when ball is launched.
...
- **Behavior:** body_entered(Ball) → remove ball from tree; count balls in Balls container; if 0 → call GameManagerV4.on_round_lost().
- **Script:** DrainV4.gd – connect body_entered to handler; get parent or autoload GameManagerV4; on_round_lost().
...
- **Kickers:** Area2D or StaticBody2D with collision; on ball contact GameManagerV4.add_score(5000); play kicker sound.
...
- **Behavior:** body_entered(Ball) → GameManagerV4.add_score(1000000); play rollover sound.
- **Script:** SkillShotV4.gd – on body_entered, if body is Ball then add_score(1000000) and play sound.
...
- **Word:** Track which letters are lit; when all lit → GameManagerV4.add_bonus("googleWord"), reset letters, trigger bonus ball (5s timer in GameManagerV4).
- **Script:** GoogleGalleryV4.gd – manage letter state; on all lit call add_bonus and request bonus ball spawn.
...
- **Behavior:** When GameManagerV4 emits bonus_activated("googleWord") or bonus_activated("dashNest"), animate indicators (blink). No ball spawning here; bonus ball is spawned by GameManagerV4 after 5s.
- **Script:** MultiballsV4.gd – connect to bonus_activated; if bonus is googleWord or dashNest, start blink animation on all four.
...
- **Ramp:** On ball entry: add_score(5000); increment ramp hit count; every 5 hits call GameManagerV4.increase_multiplier() (if multiplier < 6); if “ramp bonus” condition (e.g. N hits in round) add_score(1000000) and reset progress.
- **Bumpers:** Each bumper ball contact → add_score(20000), bumper sound.
- **Spaceship target:** Ball enters sensor → add_score(200000), GameManagerV4.add_bonus("androidSpaceship").
- **Script:** AndroidAcresV4.gd – ramp hit counter; call add_score, increase_multiplier, add_bonus as above.
...
- **Dino mouth:** body_entered(Ball) on mouth sensor → add_score(200000), GameManagerV4.add_bonus("dinoChomp").
- **DinoWalls:** Used as spawn position for bonus ball (e.g. 29.2, -24.5) with impulse (-40, 0); GameManagerV4 spawns ball there after 5s when Google Word or Dash Nest bonus.
- **Slingshots:** Visual/collision; optional slingshot behavior (bounce).
- **Script:** DinoDesertV4.gd – mouth contact → add_score(200000), add_bonus("dinoChomp"). Bonus ball spawn is in GameManagerV4.
...
- **Dash bumpers:** Each contact → add_score(20000 or 200000 for main); track “lit” state; when all three lit → GameManagerV4.add_bonus("dashNest"), trigger bonus ball (5s in GameManagerV4), reset lit state.
- **Script:** FlutterForestV4.gd – track bumper lit; on all lit add_bonus("dashNest") and request bonus ball.
...
- **Computer sensor:** body_entered(Ball) → add_score(200000), GameManagerV4.add_bonus("sparkyTurboCharge").
- **Script:** SparkyScorchV4.gd – forward contacts to add_score and add_bonus.
...
- **Behavior:** BackboxManagerV4 state drives which display is visible. On game over, request_initials(score, character); user submits → save to local leaderboard → show game over info → share option.
- **Script:** BackboxV4.gd – listens to BackboxManagerV4.state_changed; shows/hides or instantiates correct display. BackboxManagerV4 handles load_leaderboard (local), submit_initials (save), request_share (copy or native).
...
**Godot v4.0:**
- Reuse Ball.tscn/Ball.gd where possible. Spawn at LauncherV4 spawn position; add to Balls container. Character theme selects ball sprite (Sparky, Dino, Dash, Android).
...
| Behavior | Flutter | Godot v4.0 |
|----------|---------|------------|
| Scoring | ScoringContactBehavior(points) | Zone script calls GameManagerV4.add_score(points) |
| Bonus | BonusActivated(bonus) | Zone script calls GameManagerV4.add_bonus(bonus) |
| Multiplier (ramp) | RampMultiplierBehavior every 5 hits | AndroidAcresV4 ramp counter → increase_multiplier() |
| Bonus ball | BonusBallSpawningBehavior 5s, spawn at DinoWalls | GameManagerV4 5s timer → spawn Ball at (29.2,-24.5) with impulse |
| Round lost | DrainingBehavior → RoundLost | DrainV4 body_entered → if no balls → on_round_lost() |
| Ball spawn (round) | BallSpawningBehavior on round start/lost | GameManagerV4 on_round_lost (if rounds>0) or game_started → LauncherV4 spawn position |

---

## 15. Reference

- Flutter paths: [FLUTTER-PINBALL-PARSING.md](FLUTTER-PINBALL-PARSING.md) sections 4–4.12.
- Technical design: [Technical-Design-v4.0.md](Technical-Design-v4.0.md).
